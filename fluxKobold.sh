...
CHECKPOINT_MODELS=(
    "https://civitai.com/api/download/models/1685122?type=Model&format=SafeTensor&size=full&fp=fp16",
    "https://civitai.com/api/download/models/1699918?type=Model&format=SafeTensor&size=pruned&fp=fp16"
)

LORA_MODELS=(
    #"https://civitai.com/api/download/models/16576",
    "https://civitai.com/api/download/models/1158586?type=Model&format=SafeTensor"
)
...
KOBOLD_MODELS=(
    "https://huggingface.co/ArliAI/QwQ-32B-ArliAI-RpR-v3-GGUF/resolve/main/QwQ-32B-ArliAI-RpR-v3-Q4_K_M.gguf"
)
...
function provisioning_start() {
    ...
    provisioning_get_models \
        ...
        "${CLIP_MODELS[@]}"
    provisioning_get_models \
        "${WORKSPACE}/storage/koboldcpp/models" \
        "${KOBOLD_MODELS[@]}"
...
function provisioning_download() {
    ...
    if [[ -f "$output_path" && "${FORCE_REDOWNLOAD:-false}" != "true" ]]; then
        echo "File already exists at $output_path. Skipping download (set FORCE_REDOWNLOAD=true to override)."
        return 0
    fi
